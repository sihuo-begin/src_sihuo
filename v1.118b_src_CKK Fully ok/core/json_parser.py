...
def export_to_excel(df: pd.DataFrame, output_path: str,
    ...
    # Sort: part_num → appraiser → trial
    sort_cols = ["part_num", "appraiser", "trial"]
    df_out.sort_values(
...
class JsonParser:
    ...
    def _write_specs_sidecar(self, excel_path: str):
        ...
        if df is None:
            return
        lsl_usl = {c: df[c].dropna().iloc[0] if len(df[c].dropna()) > 0 else None
                   for c in df.columns if c.endswith("_lsl") or c.endswith("_usl")}
        if not lsl_usl:
            return
