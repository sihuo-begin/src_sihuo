...
class DataLoader:
    ...
    def _to_mt7_format(self) -> pd.DataFrame:
        ...
        # IMPORTANT: iterate op candidates in priority order and pick the
        # first matching COLUMN NAME (not the first column in the sheet
        # that matches any candidate). This prevents the "QR_SCAN wins"
        # bug when the sheet has columns in order [QR_SCAN, ..., appraiser].
        sn_col = None
        for candidate in ("sample", "sn", "serial", "device", "lot", "pn"):
            for c in cols:
            ...
            if sn_col:
                break
        op_col = None
        for candidate in ("appraiser", "inspector", "operator", "qr_scan", "tester"):
            for c in cols:
                if c.lower() == candidate:
                    op_col = c
                    break
