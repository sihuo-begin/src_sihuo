...
from utils.config import (
    LED_SPECS, PNUM_MAPPING, _norm_led,
    GRR_CHART_LAYOUT, GRR_SHEET_NAMING, GRR_TEMPLATE_PATH,
    GRR_FORM_DEFAULTS, GRR_LAYOUT,
)
...
class ExcelReportGenerator(JsonParser):
    ...
    def __init__(self, output_dir: str = "output",
                 template_path: Optional[str] = GRR_TEMPLATE_PATH):
    ...
    def _load_or_create_template(self) -> openpyxl.Workbook:
        ...
        if self.template_path and Path(self.template_path).exists():
            try:
                ...
                logger.warning("Failed to load GRR template %s: %s. Falling back to blank.",
                               self.template_path, e)
        self._used_template = False
        wb = openpyxl.Workbook()
        wb.remove(wb.active)
        logger.info("GRR template not found at %s; using blank workbook.",
                    self.template_path)
        return wb
    ...
    def generate(self, results: Dict, df: pd.DataFrame,
        ...
        if self._used_template:
            ...
            for it in grr_items:
                pnum_raw = LED_SPECS.get(it, {}).get("pnum", it[-4:])
    ...
    def _make_grr_data_page(self, wb, df: pd.DataFrame, inspector_numbers: List[str]):
        ...
        for row_idx, insp in enumerate(inspector_numbers[:3], start=6):
            ...
            _apply_header(ws.cell(row_idx, 1), "Inspector Number:",
                          bg=CLR_SECTION_BG, align="left", fg="1F4E79")
    ...
    def _make_pnum_page(self, wb, item: str, grr_res,
                         spec: dict, chart_paths: Optional[dict] = None):
        ...
        safe = item.replace("/", "_")
        item_chart_keys = [k for k in (chart_paths or {}).keys()
                           if k.startswith(safe + "__")]
        ...
        if chosen:
            try:
                ...
                if hasattr(ws, "_images") and ws._images:
                    logger.info("Clearing %d pre-existing images on sheet '%s'",
                                len(ws._images), sheet_name)
                    ws._images.clear()
                ...
                for idx, (chart_key, chart_src) in enumerate(chosen):
                    ...
                    img.anchor = anchor
                    ws.add_image(img)
                    logger.info("GRR chart #%d ('%s'/%s) embedded on sheet '%s' at R%dC%d (%dx%d)",
                                idx + 1, getattr(chart_src, 'name', os.path.basename(str(chart_src))), layout_key,
                                sheet_name, row, col, w, h)
    ...
    def _update_pnum_page(self, ws, item: str, grr_res, spec: dict,
        ...
        if hasattr(ws, "_images") and ws._images:
            logger.info("Clearing %d old images on template sheet '%s'",
                        len(ws._images), ws.title)
            ws._images.clear()
        ...
        try:
            ...
            for img_name, col, row, w, h, colOff_px, rowOff_px in GRR_LAYOUT:
                ...
                placed += 1
            if placed:
                logger.info("GRR sheet '%s': embedded %d chart(s) via GRR_LAYOUT",
                            ws.title, placed)
        except Exception as e:
