...
from utils.config import LED_SPECS, OUTPUT_DIR
...
class CPKExcelReportGenerator(JsonParser):
    """
    ...
    def _make_data_sheet(self, wb, df: pd.DataFrame, product_name: str = "Beta Charger"):
        ...
        # ── Strategy: 1:1 copy of the raw DataFrame as parsed from the
        # Jason log (via JsonParser). The user wants the CPK report's
        # raw data sheet to reflect exactly what the parser produced —
        # no column rename, no metadata-column blacklist, no dtype
        # coercion. Header row uses the original column names from the
        # intermediate Excel; values are written as-is (preserving the
        # string dtype that DataLoader uses).
        n_cols = len(df.columns)
        if n_cols == 0:
            # Empty df — still create the sheet with a placeholder
            _hdr(ws.cell(1, 1), "sn", bg=CLR_SUBHDR_BG, fg=CLR_SUBHDR_FG,
                 size=9, align="center")
            ws.row_dimensions[1].height = 28
            ws.freeze_panes = "A2"
            return

        # Column widths: first column wider (SN), rest moderate
        ws.column_dimensions["A"].width = 24
        for j in range(2, n_cols + 1):
            ws.column_dimensions[get_column_letter(j)].width = 16

        # Header row = original column names from the raw data
        for j, h in enumerate(df.columns, start=1):
            _hdr(ws.cell(1, j), str(h) if h is not None else "",
                 bg=CLR_SUBHDR_BG, fg=CLR_SUBHDR_FG,
                 size=9, align="center")
        ws.row_dimensions[1].height = 28
        ws.freeze_panes = "A2"

        # Drop all-None rows from df before writing
        df_data = df.dropna(how='all').reset_index(drop=True)

        # Write data rows starting at row 2 (immediately after header),
        # preserving the original dtype / string representation.
        for i, (_, row) in enumerate(df_data.iterrows()):
            ...
            for j, col in enumerate(df.columns, start=1):
                ...
                if val is None or (isinstance(val, float) and np.isnan(val)):
                    ...
                    cell_val = str(val)  # keep as-is (string), no coercion
                _dat(ws.cell(r, j), cell_val, bg=bg, fmt="General", align="left")
    ...
    def _make_cpk_sheet(self, wb, results: Dict, inline_charts: bool = False):
        ...
        # Pull each item's PNUM from LED_SPECS (the canonical mapping in utils/config.py).
        # Falls back to the last 4 characters of the item name if a spec entry is missing
        # (matches the GRR report's fallback for items outside the standard LED set).
        def _pnum_for(item: str) -> object:
            # spec = LED_SPECS.get(item, {})
            # logger.info("simon LED spec is {}".format(item))
            # raw = spec.get("pnum", item[-4:])
            ...
            for data in test_data:
                if item.lower() == data["reference"].lower():
                    ...
                    break
            return raw
        cpk_items = [(_pnum_for(item), item) for item in sorted(results.keys())]
        for i, (pnum, item) in enumerate(cpk_items):
            ...
            logger.info("results details is \n cpk{}, ".format(cpk_val))
