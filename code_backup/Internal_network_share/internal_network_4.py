...
<head>
  ...
  <style>
    ...
    .box { background:#fff; border:1px solid #e5e5e5; padding:16px; border-radius:10px; margin-bottom:14px; }
...
# ======== 业务函数 ========
# def list_files(q: str | None = None):
def list_files(q: str or None = None):
    ...
    for p in paths:
        ...
        if ql and (ql not in name.lower()) and (ql not in station.lower()):
            continue
        items.append({
            "name": name,
            "station": station,
